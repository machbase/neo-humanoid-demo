...
- To run a `.js` file manually in this project, use the Machbase Neo JSH shell. Run project scripts from `/work`, for example `./scripts/schema.js`. Run the demo server from `/work/app` with `./server.js --host 127.0.0.1 --port 56802`.
- Assume the repository is installed under the Machbase Neo public package directory and is visible in JSH as `/work`.
