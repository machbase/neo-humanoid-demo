...
function episodeByIndex(conn, dataset, sequence, index) {
  ...
  const rows = queryAll(
    conn,
    `SELECT time, value, frame_id FROM ${TABLES.timeline} WHERE name = ${sqlString(tagName)} ORDER BY frame_id LIMIT ${EPISODE_INDEX_LIMIT}`
  );
  const items = episodeIndexFromRows(rows || []);
...
function pointsBinary(args, query) {
  ...
  try {
    return withDb(args, (conn) => {
      ...
      const rows = queryAll(
        conn,
        `SELECT value, point_count, byte_count, source_episode, source_step, frame_id FROM ${TABLES.points} WHERE name = ${sqlString(tagName)} AND frame_id <= ${sqlNumber(fr.frameId, 0)} ORDER BY frame_id DESC LIMIT 1`
      );
      if (!rows || rows.length === 0) return fallbackPointBytes(Number.isFinite(ms) ? ms : fallbackFrameIdMs(fr.frameId), lod);
